    ...
    def retrival(self, section, query_to_search, ranking_method, index):
        ...
        if section == "body":
            ...
            if 3>len(query_to_search)>1:
                more_tokens = list(map(lambda x: x[0], model100.most_similar(query_to_search, topn=2)))
            if len(query_to_search)>=3:
                more_tokens = list(map(lambda x: x[0], model.most_similar(query_to_search, topn=2)))
