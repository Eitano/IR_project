...
class MultiFileWriter:
    ...
    def __init__(self, base_dir, name, bucket_name, ftype=""):
        self._base_dir = Path(base_dir)
        self._name = name
        self._file_gen = (open(self._base_dir / f'{name}_{i:03}{str(ftype)}.bin', 'wb')
...
#                 self._open_files[f_name] = open(f_name, 'rb')
                self._open_files[f_name] =open(Path('./postings_gcp/postings_gcp') / f'{f_name}', 'rb')
...
class InvertedIndex:  
    ...
    @staticmethod
    def write_a_posting_list(b_w_pl, bucket_name, ftype=""):
        posting_locs = defaultdict(list)
        bucket_id, list_w_pl = b_w_pl
        
        with closing(MultiFileWriter(".", bucket_id, bucket_name, ftype=ftype)) as writer:
